  ...
  $.getJSON(
    ...
    function (api) {
      ...
      $(".icon").attr(
        "src",
        changeIcon(
          api.current.weather[0].id,
          api.current.weather[0].icon.substring(2)
        )
      );
...
function setWeather(api, prev_date) {
  console.log(api);
  // getting the temp from the previous day
  let temp = Math.floor(api.current.temp);
  $(".temp").html(temp + "&deg;F");
...
// END OF CODE THAT DREW IMPLEMENTED
