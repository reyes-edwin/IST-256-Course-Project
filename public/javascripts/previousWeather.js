...
// Get weather data using API call
function getWeather() {
    ...
    // Using the data from above to retrieve the weather data
    $.ajax({
        method: "GET",
        url: "/weather/coords/previous",
        data: {
            lat: lat,
            lon: lon,
            date: previous,
        ...
        async: false,
        success: function(data) {
            setWeather(data, prev_date, location);
        },
        error: function(xhr, status, error) {
            var errorMessage = xhr.status + ": " + xhr.statusText;
            console.log("Error: " + errorMessage);
...
function setWeather(data, prev_date, locationInfo) {
    ...
    let location = locationInfo.cityName;
    $(".city").html(location + " Previous weather");
    ...
    //   selecting the icon class to change the icon  img based on the weather condition
    $(".icon").attr(
        "src",
        changeIcon(
            data.current.weather[0].id,
            data.current.weather[0].icon.substring(2)
        )
    );
