...
function runAPI() {
    $.ajax({
        ...
        url: "https://api.openweathermap.org/data/2.5/onecall?lat=" +
            ...
            "&exclude=daily,minutely,alerts&units=imperial&appid=c0c35b925bbddaaf1dca134adf31f13a",
...
function populateWeather(obj) {
    for (var i = 0; i < 6; i++) {
        ...
        $(iId).attr(
            "src",
            changeIcon(hour.weather[0].id, hour.weather[0].icon.substring(2))
        );
        if (hour.weather[0].main === "Clear") {
            ...
            hour.weather[0].main === "Fog" ||
            hour.weather[0].main === "Mist"
        ) {
            $(hId).css("background", "#78777F");
        }

        nData.push(
            convertToDate(hour.dt).toLocaleTimeString([], {
                hour: "numeric",
                minute: "numeric",
                hour12: true,
            })
